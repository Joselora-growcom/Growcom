...
import AutomationLandingPage from "./AutomationLandingPage";

export const AUTOMATION_EXPERIENCE_LAYOUT_ID = "growcom-automation-experience";
const LAYOUT_ID = AUTOMATION_EXPERIENCE_LAYOUT_ID;

type AutomationExperiencePortalProps = {
  open: boolean;
  onClose: () => void;
};

export default function AutomationExperiencePortal({ open, onClose }: AutomationExperiencePortalProps) {
  const { t } = useI18n();
  const portal = t.landing.automation.portal;
      ...
      {open ? (
        <>
          ...
          >
            ...
            >
              <AutomationLandingPage embedded />
            </motion.div>
          </motion.div>
        </>
      ) : null}
